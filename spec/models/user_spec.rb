...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    context '新規登録できるとき' do
      it 'email,password,nickname,birthday,姓名、姓名（カナ）が存在すれば登録できる' do
    ...
    context '新規登録できないとき' do
      ...
      it '姓が全角（漢字・ひらがな・カタカナ）が空では登録できない' do
        @user.last_name = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name can't be blank")
      end
      it '名が全角（漢字・ひらがな・カタカナ）が空では登録できない' do
        @user.first_name = ''
        @user.valid?
      ...
      it '姓カナが全角（カタカナ）が空では登録できない' do
        @user.last_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana can't be blank")
      end
      it '名カナが全角（カタカナ）が空では登録できない' do
